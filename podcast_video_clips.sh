...
ffmpeg -i "$AUDIO_FILE" -filter_complex movie=".temp/$SHOW_NAME.$EPISODE_NO.$EPISODE_NAME.$FCLIP_NAME.loop.16x9.mp4":loop=0,setpts=N/FRAME_RATE/TB -shortest "$SHOW_NAME.$EPISODE_NO.$EPISODE_NAME.$FCLIP_NAME.16x9.noloop.mp4"
ffmpeg -i "$SHOW_NAME.$EPISODE_NO.$EPISODE_NAME.$FCLIP_NAME.16x9.noloop.mp4" -filter_complex \
 "[0:v]null[bg]; \
 [0:a]showwaves=mode=line:s=1000x800:colors=#00000099[fg]; \
 [bg][fg]overlay=x=920:y=250" "$SHOW_NAME.$EPISODE_NO.$EPISODE_NAME.$FCLIP_NAME.16x9.mp4"
