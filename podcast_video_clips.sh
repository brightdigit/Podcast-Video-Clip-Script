...
CLIP_NAME="Growth and Consistency"
LOGO_PATH=brightdigit.png
ANIMATED_LOGO_FORMAT=Logo-%d.png
AUDIO_FILE="Empower Apps 14 quote 1.mp3"
...
ffmpeg -i "$AUDIO_FILE" -filter_complex movie="$SHOW_NAME.$EPISODE_NO.$EPISODE_NAME.$CLIP_NAME.loop.16x9.mp4":loop=0,setpts=N/FRAME_RATE/TB -shortest "$SHOW_NAME.$EPISODE_NO.$EPISODE_NAME.$CLIP_NAME.16x9.mp4"
...
ffmpeg -i "$AUDIO_FILE" -filter_complex movie="$SHOW_NAME.$EPISODE_NO.$EPISODE_NAME.$CLIP_NAME.loop.1x1.mp4":loop=0,setpts=N/FRAME_RATE/TB -shortest "$SHOW_NAME.$EPISODE_NO.$EPISODE_NAME.$CLIP_NAME.1x1.mp4"