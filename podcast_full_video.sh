...
yes | convert -gravity East -font "$FONT" -pointsize 52 label:"$SHOW_NAME" -pointsize 32 label:"episode $EPISODE_NO" -pointsize 80 label:"$EPISODE_NAME" -pointsize 64 label:"$GUEST_APPEND" -append ".temp/$SHOW_NAME.$EPISODE_NO.$EPISODE_NAME.text.16x9.png"
...
ffmpeg -i "$AUDIO_FILE" -filter_complex movie=".temp/$SHOW_NAME.$EPISODE_NO.$EPISODE_NAME.loop.16x9.mp4":loop=0,setpts=N/FRAME_RATE/TB -shortest "$SHOW_NAME.$EPISODE_NO.$EPISODE_NAME.16x9.noloop.mp4"
ffmpeg -i "$SHOW_NAME.$EPISODE_NO.$EPISODE_NAME.16x9.noloop.mp4" -filter_complex \
 "[0:v]null[bg]; \
